...
test('Can get all 10 teams', async () => {
    let response = await teamController.getAllTeams() 
    expect( response.length ).toEqual( 10 )
})

test('Can get one team as string, eade', async () => {
    let response = await teamController.getTeamsById('eade')
...
test('Can get three teams, eade,thti,thsc', async () => {
    let response = await teamController.getTeamsById(['eade','thti','thsc']) 
...
test('Can get two teams, with one error, eade,xyz,thti', async () => {
    let response = await teamController.getTeamsById(['eade','xyz','thti']) 
...
test('Can get all teams if given * for ID', async () => {
    let response = await teamController.getTeamsById(['*']) 
    expect( response.length ).toEqual( 10 )
})
...
test('Can get all teams if given null ID', async () => {
    let response = await teamController.getTeamsById(null) 
    expect( response.length ).toEqual( 10 )
})

test('Get nothing if passed non-string/non-array', async () => {
    let response = await teamController.getTeamsById(123) 
